    ...
    <div class="row">
        <form method="post" action="" enctype="multipart/form-data">
            <div class="form-group">
                <label>
                    <input type="radio" name="optionsInput" id="optionsFileIP" value="1" <?php if ($optionsInput == 1) {
                        echo "checked";
                    } ?> >
                    Nhập dữ liệu từ File:
                </label>
                <br>
                <div id="fileIP" style="margin-left: 30px; " <?php if ($optionsInput == 2) {
                    echo "hidden";
                } ?> >
                    <p>Định dạng file:</p>
                    <p>- Dòng đầu tiên là thời gian đến của các tiến trình - Thời gian đến là số nguyên và cách nhau 1
                        dấu cách.</p>
                    <p>- Dòng thứ 2 là thời gian chạy tương ứng của các tiến trình - Thời gian chạy là số nguyên và cách
                        nhau 1 dấu cách.</p>
                    <p>- Nếu sử dụng thuật toán Round Robin thì dòng thứ 3 là lượng tử thời gian.</p>
                    <label>
                        Chọn file (định dạng *.txt, tối đa 100KB):
                        <input type="file" id="filedata" name="filedata" <?php if ($optionsInput == 1) {
                            echo "required";
                        } ?> >
                    </label>
                    <?php
                ...
                <label>
                    <input type="radio" name="optionsInput" id="optionsFormIP" value="2" <?php if ($optionsInput == 2) {
                        echo "checked";
                    } ?>>
                    Nhập dữ liệu từ Form:
                </label>
            </div>
            <div id="frmIP" <?php if ($optionsInput == 1) {
                echo "hidden";
            } ?> >
                <div class="form-group">
                    <label>Thời gian đến của các tiến trình - Thời gian là số nguyên và cách nhau 1 dấu cách.</label>
                    <input type="text" class="form-control" id="txtAT" name="txtAT" <?php if ($optionsInput == 2) {
                        echo "required";
                    } ?> autofocus placeholder="Yêu cầu có tiến trình đến từ 0">
                </div>
                <div class="form-group">
                    <label>Thời gian chạy tương ứng của các tiến trình - Thời gian là số nguyên và cách nhau 1 dấu
                        cách.</label>
                    <input type="text" class="form-control" id="txtBT" name="txtBT" <?php if ($optionsInput == 2) {
                        echo "required";
                    } ?> >
                </div>
            </div>
            <div class="form-group">
                ...
                <label class="checkbox-inline">
                    <input type="checkbox" name="fcfs" value="1" <?php if ($fcfs == 1) {
                        echo 'checked='.'"checked"';
                    } ?> > FCFS (First Come First Served)
                </label>
                <br>
                <label class="checkbox-inline">
                    <input type="checkbox" name="sjnf" value="1" <?php if ($sjnf == 1) {
                        echo 'checked='.'"checked"';
                    } ?> > SJFnP (Shortest Job First non Preemptive)
                </label>
                <br>
                <label class="checkbox-inline">
                    <input type="checkbox" name="sjf" value="1" <?php if ($sjf == 1) {
                        echo 'checked='.'"checked"';
                    } ?> > SJFP (Shortest Job First Preemptive)
                </label>
                <br>
                <label class="checkbox-inline">
                    <input id="rr" type="checkbox" name="rr" value="1" <?php if ($rr == 1) {
                        echo 'checked='.'"checked"';
                    } ?> > RR (Round Robin)
                </label>
                <br>
