<div class="row">
    ...
    <h3>
        ...
        <span class="label label-info"><?php if ($isRR == 1) echo "Thời gian lượng tử: $tgLuongTu"; ?></span>

        <span class="label label-danger"><?php echo "Chờ trung bình: ".number_format($tongTgCho/$soTienTrinh,2); ?></span>
        
        <span class="label label-primary"><?php echo "Lưu trung bình: ".number_format($tongTgLuu/$soTienTrinh,2); ?></span>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <?php for ($j = 1;$j <= $soTienTrinh ; $j++){ ?>
                    <td><?php echo $arrKT[$j] - $arrBT[$j]; ?></td>
                    <?php } ?>
                    <td>X</td>
                </tr>
                <?php } ?>
        ...
        <table class="table table-bordered">
            <thead>
                <tr>
                    ...
                    <?php $curMau = 1; for ($i = 1; $i <= $dem;$i++){ 
                        if ($curMau == 21) $curMau = 1;
                        $tgChay = ($canBenPhai[$i] - $canBenPhai[$i-1]);  
                        ?>
                    <th style="text-align: center;font-size: 30px; height: 50px; min-width: <?php echo ($tgChay*37).'px'.";background-color:$mau[$curMau]"; $curMau++; ?>" ><?php echo "P$thuTu[$i]"; ?></th>
                    <?php } ?>
                </tr>
            </thead>
            <tbody>
...
<div class="row">
    ...
    <div class="progress" style="height: 50px">
        ...
        <?php $curMau = 1; for ($i = 1; $i <= $dem;$i++){ 
            ...
            ?>
        <div class="progress-bar progress-bar-success" style="width: <?php echo "$phanTram%;background-color:$mau[$curMau];"; $curMau++; ?> font-size: 30px; padding-top: 15px;">
    ...
    <div class="progress" style="height: 35px">
        ...
        <?php for ($i = 1; $i <= $dem;$i++){
            $phanTram = ($canBenPhai[$i] - $canBenPhai[$i-1])*100/$tongThoiGian;
            if ($i == 1) $phanTram += 0.85; 
            else if ($i == $soTienTrinh) $phanTram -= 0.85;
        ?>
                <div class="progress-bar progress-bar-success" style="width: <?php echo "$phanTram%;"; ?>  padding-top: 3px;background: white; color: black;font-size: 30px;">
                <?php 
                    if ($i == 1){ 
